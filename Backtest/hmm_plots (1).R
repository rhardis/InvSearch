...
close.p =  ggplot(post_probs[post_probs$date >= "2019-11-01",], aes(x=date, y=Close.Price, color=State_Name)) +
  geom_line()+
  ylab("Closing Price ($)")+
...
p <- ggplot(post_probs[post_probs$date >= "2019-11-01",], aes(x=date, y=returns, color=State_Name)) +
  geom_point()+
  ylab("log returns")+
  scale_x_date(date_labels = "%m-%Y")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
